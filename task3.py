...
def main():
    color1 = "gray"
    color2 = "pink"
    print(f"1) Roll a ball using a {color1}.\n")
    Body_choice = input("long body or round body? ")
    # CAUTION: You must include the word "body" when checking!
    if Body_choice.lower() == "long body":
    ...
    tail_choice = input("long tail or short tail? ")
    if tail_choice.lower() == "long tail":
